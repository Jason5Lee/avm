...
impl crate::tool::GeneralTool for Tool {
    ...
    fn describe_flavor(&self, flavor: &str) -> &'static str {
        match flavor {
            "jdk" => "Standard Liberica JDK.",
            "jdk_full" => "JDK with LibericaFX and Minimal VM.",
            "jdk_lite" => "Smaller JDK build optimized for size.",
            "jre" => "Standard Liberica JRE runtime.",
            "jre_full" => "JRE with LibericaFX and Minimal VM.",
            "nik_core" => "Native Image Kit core distribution.",
            "nik_standard" => "Native Image Kit with extra language plugin support.",
            "nik_full" => "Native Image Kit with LibericaFX.",
        ...
        I: Iterator<Item = (&'a str, &'a Version)>,
    {
        tags_and_versions
            .filter_map(|(tag, version_info)| {
                ...
                let version = JdkVersion::parse(raw_version);
                if !match_liberica_version_filter(
                    raw_version,
                    version,
                    version_info.is_lts,
                    version_filter,
...
- **`nik_full` (Full version):** A comprehensive build that includes LibericaFX for GUI-based applications."#.into()),
    ...
    async fn fetch_liberica_releases(
        ...
        let mut url = self.build_url(
            format!("{}liberica/releases", BASE_URL),
            args.cpu,
            args.os,
            args.bitness,
...
fn match_liberica_version_filter(
    raw_version: &str,
    version: JdkVersion,
    is_lts: bool,
    version_filter: &VersionFilter,
    ...
    if version_filter
        .version_prefix
        .is_some_and(|p| !p.matches(version.major, version.minor, version.security))
    {
        return false;
    }
    if version_filter
